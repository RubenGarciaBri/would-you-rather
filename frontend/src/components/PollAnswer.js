    ...
    <div className="poll-answer shadow-smooth">
      <div className="poll-answer__head">
        <img
          src={author.avatar}
          alt="User avatar"
          className="poll-answer__head-img shadow-xs"
        />
      ...
      <div className="poll-answer__body">
        <p className="poll-answer__body-title">Would you rather...</p>
        {
          ...
          answeredBy.includes(authedUser.id) || authedUser.id === author.id ? (
            <div>
              <p className="poll-answer__body-result__question">
                {firstQuestion}
              </p>
              <Progress
                progress="percent"
                percent={((firstQuestionReplies / totalReplies) * 100).toFixed(
                  0
                )}
                success={option === firstQuestion ? true : false}
                size="large"
              />
              <p className="poll-answer__body-result__score">
                {firstQuestionReplies} out of {totalReplies} votes
              </p>
              <p className="poll-answer__body-result__question">
                {secondQuestion}
              </p>
              <Progress
                progress="percent"
                percent={((secondQuestionReplies / totalReplies) * 100).toFixed(
                  0
                )}
              ...
              />
              <p className="poll-answer__body-result__score">
                {secondQuestionReplies} out of {totalReplies} votes
              </p>

              <Link to="/home">
                <Button className="poll-answer__body-result__btn">Back</Button>
              </Link>
            </div>
          ) : (
